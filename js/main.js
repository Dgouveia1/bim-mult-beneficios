...
import { allPeople, loadClientsData, handleNewClientSubmit, openModal, closeModal, addDependenteField, openDetailsModal, handleUpdateClient, filterAndRenderClients, exportToExcel } from './clientes.js';
...
const detailsClientModalEl = document.getElementById('detailsClientModal');
...
function loadPageData(pageName) {
    ...
    else if (pageName === 'profissionais') loadProfessionalsData(); // 2. ADICIONA O CARREGAMENTO
...
function setupEventListeners() {
    ...
    // --- BOTÕES DE AÇÃO PRINCIPAIS (não dinâmicos) ---
    // Adicionamos os listeners diretos aqui para maior confiabilidade
    ...
    document.getElementById('addUserBtn')?.addEventListener('click', () => openUserModal()); // Listener direto para "Adicionar Usuário"
    ...
    // --- DELEGAÇÃO DE EVENTOS GLOBAIS (para cliques em itens dinâmicos) ---
    document.body.addEventListener('click', function(event) {
        const target = event.target;

        // Botão para fechar qualquer modal
        if (target.closest('.close-btn') || target.closest('[data-close-modal]')) {
            const modal = target.closest('.modal');
            if(modal) modal.style.display = 'none';
        }

        // Botão "Editar Usuário" (dinâmico)
        const editUserButton = target.closest('.edit-user-btn');
        if (editUserButton) {
            openUserModal(editUserButton.dataset.id);
        }
...
async function initializeDashboard(user) {
    const { data: profile, error } = await _supabase.from('profiles').select('role').eq('id', user.id).single();
    if (error || !profile) {
...
function showDashboard() { loginScreen.style.display = 'none'; dashboard.style.display = 'flex'; }
function showLoginScreen() { loginScreen.style.display = 'flex'; dashboard.style.display = 'none'; localStorage.clear(); }

document.addEventListener('DOMContentLoaded', async () => {
    setupEventListeners();
    const { data: { session } } = await _supabase.auth.getSession();
    if (session) await initializeDashboard(session.user);
    else showLoginScreen();
});

export { initializeDashboard, showLoginScreen, showDashboard };